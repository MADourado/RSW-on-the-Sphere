...
`outputs/figures/`): `dispersion_relation.png`, `dynamics.png`, and per-mode
`mode_{a,b,c}/Hough_harmonic.png` + `derivatives.png`. `outputs/` is
gitignored and regenerated by running `run_diagnostics.py` / `run_dynamics.py`. Standalone dispersion/Hough
scripts write wherever their `path` argument points.
