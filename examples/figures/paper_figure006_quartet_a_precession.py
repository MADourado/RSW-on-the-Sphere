...
Calls run_sweep.run_sweep once for [dynamical_phase, efficiency] on the
...
unchanged. No config file needed beyond the registry entry's own
``sweep:``/``settings:`` blocks; the paper-specific framing (which mode's
efficiency to show, which triad's own frequency curve, axis labels) lives
here, not in the registry -- run_sweep.py itself has no paper-specific
concept of either.
...
TARGET_MODE_KEY = "c"    # RH(3,4) -- efficiency shown on the right axis
...
def main():
    ...
    print(f"Running sweep for wave set {WAVE_SET_KEY!r} (dynamical_phase + efficiency)...")
    ...
    # together. sweep_results['efficiency']['series'] is already a %,
    ...
    result = {
        ...
        'efficiency': sweep_results['efficiency']['series'][(target_label, 'full')] / 100,
