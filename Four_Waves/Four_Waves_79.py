...
class FOUR_WAVES:
    ...
    def __init__(self, gamma, m_a, n_a, alpha_a, m_b, n_b,
        ...
        inner_ABD = inner_product(A, m_a, B, m_b, D, m_d, deg, True)

        inner_BAC = inner_product(B, m_b, C, m_c, A, m_a, deg, True)
        inner_BAD = inner_product(B, m_b, A, m_a, D, m_d, deg, True)

        inner_CAB = inner_product(C, m_c, B, m_b, A, m_a, deg, True)

        inner_DAB = inner_product(D, m_d, B, m_b, A, m_a, deg, False)
        ...
        self.mismatch_2 = -self.freq_d + self.freq_b + self.freq_a

        self.S_abc =  -S_abc(C, m_c, B, m_b, A, m_a, deg)
        self.S_abd =  S_abc(B, m_b, A, m_a, D, m_d, deg)
    ...
    def f(self, AMP):
        ...
        F1 += 1j * coef_ABD * np.conj(A_b) * A_d

        F2 = -1j * self.freq_b * A_b + 1j * coef_BAC * A_a * np.conj(A_c)
        F2 += 1j * coef_BAD * np.conj(A_a) * (A_d)

        F3 = -1j * self.freq_c * A_c + 1j * coef_CAB * A_a * np.conj(A_b)

        F4 = -1j * self.freq_d * A_d + 1j * coef_DAB * A_a * (A_b)
...
def four_waves_integration(Four_Waves,T1,T2, A_0, t_0, t_f, h, plot = True):
    ...
    mc, mb, ma,_,_,_   = Triad_dynamics(T1, np.array([A_c, A_b, A_a]), t_0, t_f, h,p=True)
    md, mbb, maa,_,_,_ = Triad_dynamics(T2, np.array([A_d, A_b, A_a]), t_0, t_f, h,p=True)
...
def eff_graph_double(Four,T1,T2, A_0, u_c, u_d):
    ...
    plt.title(fr'$P$ measure - {Four.label_a}')
...
def eff_dif_graph(Four, T1, A_0, u_d):
    ...
    edd, ebb, eaa = Triad_dynamics(T1, np.array([A_d, A_b, A_a]), t_0, 
    ...
    plt.plot(ALPHA*u_c*2, 100*E_a, label = f'{Four.label_a}')
    plt.plot(ALPHA*u_c*2, 100*E_b, label = f'{Four.label_b}')
    plt.plot(ALPHA*u_c*2, 100*E_d, label = f'{Four.label_d}')
...
(m_a, n_a, alpha_a, m_b, n_b, alpha_b,
 m_c, n_c, alpha_c, m_d, n_d, alpha_d) =  4, 5, 3, 3, 4, 3, 1, 2, 3, 7, 9 ,1 #EEER
...
t_f = 10
k = 40
A_0 = np.array([A_a*15, A_b*15, A_c*20, A_d*30])
print(A_0)
#period_graph(Four_Waves, Triad_a, Triad_b, A_0, u_d)
four_waves_integration(Four_Waves,Triad_a, Triad_b, A_0, t_0, t_f, h)
#A,B, Ja, Jb = eff_graph_double(Four_Waves, Triad_a,Triad_b, A_0, u_c, u_d)
#eff_dif_graph(Four_Waves, Triad_b, A_0, u_d)
