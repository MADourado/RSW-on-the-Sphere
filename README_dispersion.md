...
`utils/Dispersion_relation_fancy.py` computes and plots the **dispersion relation
...
python utils/Dispersion_relation_fancy.py dispersion_exp.png

# choose a different equivalent height (metres)
python utils/Dispersion_relation_fancy.py dispersion_exp.png --he 5000

# run as a module from the repository root
python -m utils.Dispersion_relation_fancy dispersion_exp.png

# show interactively instead of saving (omit the path)
python utils/Dispersion_relation_fancy.py
```

`python utils/Dispersion_relation_fancy.py --help` prints the full argument list.
...
`Hough_Harmonics` is found). It handles this automatically — direct execution,
...
(`utils`→`Hough_Harmonics/Eigenvalues_and_eigenvectors/Matrix_system.py`).
